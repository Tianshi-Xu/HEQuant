...
parser.add_argument('--wq_per_channel', action='store_true', default=True,
                    help='per-channel quantization for weight')
    ...
    if args.qmodules is not None:
        for m in args.qmodules:
            ...
            acfg = {
                ...
                "thd_pos": args.aq_pos,
                "thd_neg": args.aq_neg,
                "all_positive": False if "conv1" in m else args.aq_asym,
                "symmetric": True if "conv1" in m else not args.aq_asym,  # not args.aq_asym,
